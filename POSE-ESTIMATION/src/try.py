...
def predict_video(data_type,data_path):
    ...
    def get_command_line_arguments():
        ...
        def parse_args():
            ...
            parser.add_argument("-t", "--data_type", required=False, default=data_type,
                                choices=["video", "folder", "webcam"])
            parser.add_argument("-p", "--data_path", required=False, default=data_path,
            ...
            parser.add_argument("-o", "--output_folder", required=False, default='src/static/',
    ...
    if __name__ == "__main__":
        ...
        #img_displayer = lib_images_io.ImageDisplayer().avi
        ...
        try:
            ...
            print("Program ends")
# -- Command-line input
def predict_webcam(data_type,data_path):
    ...
    def get_command_line_arguments():
        ...
        def parse_args():
            ...
            parser.add_argument("-p", "--data_path", required=False, default=data_path,
                                ...
                                "absolute or relative to this project's root. "
                                "For webcam, either input an index or device name. ")
            parser.add_argument("-o", "--output_folder", required=False, default='static/',
                                help="Which folder to save result to.")
...
def avi_to_mp4(avi_file_path, output_name):
...
def webcam():
    if request.method == 'POST':
        datatype=request.form['start'];
        if datatype == 'Start':
            predict_webcam(data_type = 'webcam',data_path = '0')
...
def video():
    if request.method == 'POST':
        ...
        link = os.path.split(path) 
        shutil.copy(path,'E:\\flask\\main\\src\\static')
        cache['link'] = link[1]
        print(cache['link'])
        predict_video(data_type='video',data_path='src\\static\\'+cache['link'])
        out_dir = os.path.splitext(link[1]) 
        cache['try_link'] = 'static/'+out_dir[0] +"/"+ 'video.avi'
        print(cache['try_link'])
        #avi_to_mp4( cache['try_link'] , 'video' )
        cache['out_link'] = out_dir[0] + "/" + 'video.mp4'
        print(cache['out_link'])
...
def video1():
    ...
    return render_template('video1.html' , link=cache['link'], out_link = cache['out_link'])
